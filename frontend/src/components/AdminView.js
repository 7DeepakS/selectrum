...
// --- Reusable Modal Component for Editing Events (Unchanged and Correct) ---
const EditEventModal = ({ isOpen, onClose, event, onSave, distinctDepartments, distinctSemesters, distinctSections }) => {
    ...
    const semesterOptions = useMemo(() => distinctSemesters.filter(d => d !== 'all').map(s => ({ value: s, label: s })), [distinctSemesters]);
    const sectionOptions = useMemo(() => distinctSections.filter(d => d !== 'all').map(s => ({ value: s, label: s })), [distinctSections]);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
            <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-lg">
                ...
                <div className="space-y-4">
                    <div><label htmlFor="eventName" className="block text-sm font-medium text-gray-700">Event Name</label><input type="text" id="eventName" name="name" value={formData.name} onChange={handleChange} className="mt-1 block w-full p-2 border border-gray-300 rounded-md"/></div>
                    <div><label htmlFor="maxCourses" className="block text-sm font-medium text-gray-700">Max Courses Per Student</label><input type="number" id="maxCourses" name="maxCoursesPerStudent" value={formData.maxCoursesPerStudent} onChange={handleChange} className="mt-1 block w-full p-2 border border-gray-300 rounded-md"/></div>
                    <div>
                        <label htmlFor="allowedDepartments" className="block text-sm font-medium text-gray-700">Allowed Departments</label>
                        <Select inputId="allowedDepartments" isMulti name="allowedDepartments" options={departmentOptions} value={formData.allowedDepartments} onChange={handleSelectChange} placeholder="Leave empty for all"/>
                    </div>
                    <div>
                        <label htmlFor="allowedSemesters" className="block text-sm font-medium text-gray-700">Allowed Semesters</label>
                        <Select inputId="allowedSemesters" isMulti name="allowedSemesters" options={semesterOptions} value={formData.allowedSemesters} onChange={handleSelectChange} placeholder="Leave empty for all"/>
                    </div>
                    <div>
                        <label htmlFor="allowedSections" className="block text-sm font-medium text-gray-700">Allowed Sections</label>
                        <Select inputId="allowedSections" isMulti name="allowedSections" options={sectionOptions} value={formData.allowedSections} onChange={handleSelectChange} placeholder="Leave empty for all"/>
                    </div>
                    <div className="flex items-center space-x-6 pt-2"><label htmlFor="isOpen" className="flex items-center space-x-2 cursor-pointer"><input type="checkbox" id="isOpen" name="isOpen" checked={formData.isOpen} onChange={handleChange} /><span>Open for Enrollment</span></label><label htmlFor="isViewOnly" className="flex items-center space-x-2 cursor-pointer"><input type="checkbox" id="isViewOnly" name="isViewOnly" checked={formData.isViewOnly} onChange={handleChange} /><span>View-Only Mode</span></label></div>
                </div>
...
function AdminView() {
  ...
  const [loadingStates, setLoadingStates] = useState({ initialData: true, fetchingEventEnrollmentStatus: false, isFetchingLogs: false, isDownloadingLogs: false, isDownloadingChoiceReport: false });
  ...
  const [reportSelectedColumns, setReportSelectedColumns] = useState(Object.entries(reportColumnConfig).reduce((acc, [key, val]) => ({ ...acc, [key]: val.defaultChecked }), {}));
  ...
  const setTimedMessage = (type, message, duration = 7000) => { setUiMessages({ error: '', success: '', info: '', [type]: message }); setTimeout(() => setUiMessages(prev => ({ ...prev, [type]: '' })), duration); };
  ...
  }, [logPagination.currentPage, fetchActivityLogs, loadingStates.initialData]);
  ...
  const handleApplyLogFilters = () => {
    if (logPagination.currentPage !== 1) {
      setLogPagination(p => ({ ...p, currentPage: 1 }));
    } else {
      fetchActivityLogs(1);
    }
  };
      ...
      <div className="space-y-8 p-4 md:p-6">
        ...
        <section className="p-6 bg-white rounded-xl shadow-lg">
            ...
            <div className="flex-1 mb-4 sm:mb-0">
                ...
                <select id="eventStatusTableSelect" name="eventStatusTableSelect" value={selectedEventForStatusTable} onChange={(e) => setSelectedEventForStatusTable(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500">
          ...
          <div className="p-4 border-2 border-indigo-200 rounded-lg bg-indigo-50 space-y-6">
              <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                      {Object.entries(reportColumnConfig).map(([key, val]) => (
                          <label key={key} htmlFor={`col-${key}`} className="flex items-center space-x-2 p-2 rounded-md hover:bg-indigo-100 cursor-pointer">
                              <input type="checkbox" id={`col-${key}`} name={key} checked={!!reportSelectedColumns[key]} onChange={() => handleColumnToggle(key)} className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"/>
                              <span>{val.label}</span>
                          </label>
                      ))}
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div>
                      ...
                      <select id="reportFilterEvent" name="reportFilterEvent" value={reportFilterEvent} onChange={(e) => setReportFilterEvent(e.target.value)} className="w-full p-2 border border-gray-300 rounded-lg">
                  ...
                  <div>
                      ...
                      <select id="reportFilterCourse" name="reportFilterCourse" value={reportFilterCourse} onChange={(e) => setReportFilterCourse(e.target.value)} disabled={reportFilterEvent === 'all'} className="w-full p-2 border border-gray-300 rounded-lg disabled:bg-gray-100">
                  ...
                  <div>
                      ...
                      <select id="reportFilterDept" name="reportFilterDept" value={reportFilterDepartment} onChange={(e) => setReportFilterDepartment(e.target.value)} className="w-full p-2 border border-gray-300 rounded-lg">
                  ...
                  <div>
                      ...
                      <select id="reportFilterSemester" name="reportFilterSemester" value={reportFilterSemester} onChange={(e) => setReportFilterSemester(e.target.value)} className="w-full p-2 border border-gray-300 rounded-lg">
                  ...
                  <div>
                      ...
                      <select id="reportFilterSection" name="reportFilterSection" value={reportFilterSection} onChange={(e) => setReportFilterSection(e.target.value)} className="w-full p-2 border border-gray-300 rounded-lg">
              ...
              <div>
                  <h3 className="text-lg font-semibold text-gray-700 mb-2">3. Select Enrollment Choice</h3>
                   <select id="reportChoiceNumber" name="reportChoiceNumber" value={reportChoiceNumber} onChange={(e) => setReportChoiceNumber(e.target.value)} className="w-full sm:w-auto p-3 border border-gray-300 rounded-lg">
        ...
        <section className="p-6 bg-white rounded-xl shadow-lg">
          ...
          <div className="p-4 mb-4 bg-gray-50 rounded-lg border flex flex-col sm:flex-row gap-4 items-center flex-wrap">
              <div className="flex-grow w-full sm:w-auto"><label htmlFor="logFilterUsername" className="text-sm font-medium">Username</label><input id="logFilterUsername" type="text" name="username" value={logFilters.username} onChange={handleLogFilterChange} className="mt-1 w-full p-2 border rounded-md focus:ring-2 focus:ring-blue-500" autoComplete="off"/></div>
              ...
              <div className="flex-shrink-0 pt-6 flex space-x-2">
                   <button onClick={handleApplyLogFilters} className="p-2 h-10 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors" disabled={loadingStates.isFetchingLogs}>{loadingStates.isFetchingLogs ? 'Searching...' : 'Search'}</button>
                   <button onClick={handleDownloadLogs} className="p-2 h-10 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors" disabled={loadingStates.isDownloadingLogs}>{loadingStates.isDownloadingLogs ? 'Downloading...' : 'Download'}</button>
              </div>
          </div>
          <div className="overflow-x-auto">
