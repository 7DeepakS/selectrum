...
router.get('/', authMiddleware, async (req, res, next) => {
  try {
    ...
    // --- THE FIX IS HERE ---
    // The previous code was missing the full data processing step.
    const enrichedEvents = openEvents.map(event => {
      ...
      // This block is crucial for calculating prereqs and previous enrollments
      ...
      // Return the full object with the processed course data included
      return { 
        ...event, 
        courses: processedCourses, // This was the missing piece
        isEnrolledInEvent: enrollmentsInThisEvent.length > 0,
        enrolledCourseTitles, 
        numEnrolledInEvent: enrollmentsInThisEvent.length, 
